...
export default function FeeStructureModal({ existingStructure, onClose, onSaved }) {
  ...
  const handleCreateHead = async () => {
    ...
    if (!name) return;
    const created = await createFeeHead.mutateAsync({ name });
    setFeeHeadId(created.id);
    setNewHeadName('');
  };
          ...
          <div>
            ...
            <div className="flex gap-2 mt-1.5">
              <input
                ...
                onKeyDown={(e) => e.key === 'Enter' && handleCreateHead()}
                placeholder="New fee head name..."
                className="flex-1 px-3 py-1.5 rounded-lg border border-gray-200 text-xs focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/30"
              />
              <button
                type="button"
                onClick={handleCreateHead}
                disabled={!newHeadName.trim() || createFeeHead.isPending}
                className="px-3 py-1.5 rounded-lg text-xs font-medium bg-gray-100 hover:bg-gray-200 text-gray-600 transition disabled:opacity-50"
              >
                {createFeeHead.isPending ? '...' : '+ Add'}
              </button>
            </div>
          </div>
