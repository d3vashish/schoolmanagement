...
export default function Topbar() {
  ...
  const createYear = async (e) => {
    ...
    try {
      ...
      if (res?.data?.id) setSelectedYear(res.data.id);
      setShowCreateYear(false); setShowYearDropdown(false);
      setYn({ name: '', start_date: '', end_date: '' });
    } catch (err) {
      const d = err?.response?.data?.detail;
      setYearErr(Array.isArray(d) ? d.map(x => x.msg).join(', ') : (d || 'Could not create academic year'));
    } finally {
      setSavingYear(false);
    }
  };
  ...
  return (
    <header className="flex items-center justify-between px-6 animate-in-down"
      ...
      {/* Create academic year modal — portalled to body */}
      {showCreateYear && createPortal(
        <>
          <div className="fixed inset-0 bg-black/30" style={{ zIndex: 10000 }} onClick={() => setShowCreateYear(false)} />
          <div style={{ position: 'fixed', left: '50%', top: '50%', transform: 'translate(-50%, -50%)', zIndex: 10001 }}
            className="w-[340px] bg-white rounded-2xl shadow-xl border border-[#E2E8F0] p-5">
