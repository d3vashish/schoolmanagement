...
from app.modules.timetable.schemas import TimetableSlotCreate, TimetableSlotResponse, TimetableSlotUpdate
...
def _build_slots_query():
    return (
        select(
            TimetableSlot,
            (StaffProfile.first_name + " " + StaffProfile.last_name).label("instructor_name"),
            Subject.name.label("subject_name"),
        )
        .outerjoin(StaffProfile, StaffProfile.id == TimetableSlot.instructor_id)
...
async def list_slots(
    ...
    if instructor_id:
        q = q.where(TimetableSlot.instructor_id == instructor_id)
...
async def update_timetable_slot(slot_id: str, body: TimetableSlotUpdate, db: AsyncSession = Depends(get_db)):
    slot = await update_slot(slot_id, body.model_dump(exclude_none=True), db)
    q = _build_slots_query().where(TimetableSlot.id == slot.id)
    result = await db.execute(q)
    row = result.first()
    return _row_to_response(row)
